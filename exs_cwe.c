...
int AddWordToVocab(char *word, int is_non_comp) {
  ...
  for (i = 0; i < len; i++) {
    if (cwe_type == 1 || cwe_type == 3 || cwe_type == 5) {
      ...
      //due to position information, 4 is multiplied
      if (len == 1)
        pos = 0;//this word has only 1 character
      else if (i == 0)
        pos = 1;//begin
      else if (i == len - 1)
        pos = 2;//last
...
void LearnNonCompWord() {
  ...
  while (1) {
    ...
    i = SearchVocab(word);
    if (i == -1) {
      a = AddWordToVocab(word, 1);  //1 means non-comp
      vocab[a].cn = 0;
    }
...
void LearnVocabFromTrainFile() {
  ...
  if (strlen(non_comp)) LearnNonCompWord();//non_comp is the non compositonal words file name
  while (1) {
    ReadWord(word, fin);
    if (feof(fin)) break;
    train_words++;
    if ((debug_mode > 1) && (train_words % 100000 == 0)) {
      ...
      fflush(stdout);
    }//debug mode : print some hint information
    i = SearchVocab(word);
    if (i == -1) {
  ...
  if (!cwe_type) {
	  charcn = (int *)calloc(character_size, sizeof(int));
	  for (int i = 0; i < vocab_size; ++i)
		  for (int j = 0; j < vocab[i].character_size; ++j)
				  charcn[vocab[i].character[j]]++;
  }
  SortVocab();
  if (debug_mode > 0) {
...
void InitNet() {
  ...
  a = posix_memalign((void **)&weit, 128, (long long)vocab_size * dim * sizeof(real));
    ...
    if (cbow) {  //train the cbow architecture
      ...
      for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
        ...
        if (cwe_type && vocab[last_word].character_size) {
		  ...
		  char_ave_cn /= vocab[last_word].character_size;
		  weight += wei[dim * 2 + 1] * char_ave_cn;
