...
struct vocab_word {
  long long cn;
  int *point, *character, *char_ids, character_size, *character_emb_select;
...
int cwe_type = 2, multi_emb = 3, *embed_count, cwin = 5, ada_com = -1;
...
real *syn0, *syn1, *syn1neg, *expTable, *charv, *weit;
clock_t start;
...
int AddWordToVocab(char *word, int is_non_comp) {
  ...
  vocab[vocab_size - 1].char_ids = calloc(len, sizeof(int));
...
void InitNet() {
  ...
  a = posix_memalign((void **)&weit, 128, (long long)(vocab_size + character_size + 100) * sizeof(real));
  if (syn0 == NULL || weit == NULL) {printf("Memory allocation failed\n"); exit(1);}
  if (hs) {
  ...
  for (a = 0; a < vocab_size + character_size + 100; a++){
    next_random = next_random * (unsigned long long) 25214903917 + 11;
    weit[a] = (((next_random & 0xFFFF) / (real) 65536) - 0.5) / dim;
  }
...
void *TrainModelThread(void *id) {
  long long a, b, d, cw, cwcp, t1, t2, word, last_word, sentence_length = 0, sentence_position = 0, charv_id;
  ...
  real *cvec = (real *)calloc(dim * 2 * window, sizeof(real));
  real *nvec = (real *)calloc(dim * 2 * window, sizeof(real));
  real *weights = (real *)calloc(2 * window, sizeof(real));
  ...
  while (1) {
	  ...
	  for (c = 0; c < dim * 2 * window; c++) cvec[c] = 0;
	  for (c = 0; c < dim * 2 * window; c++) nvec[c] = 0;
	  for (c = 0; c < 2 * window; c++) weights[c] = 0;
	  ...
	  if (cbow) {  //train the cbow architecture
		  ...
		  for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
			  ...
			  for (c = 0; c < dim; c++) {
				  neu1char[c] = syn0[c + last_word * dim];
				  nvec[c + cw * dim] = syn0[c + last_word * dim];
			  }
			  ...
			  if (cwe_type && vocab[last_word].character_size) {
				  ...
				  for (c = 0; c < vocab[last_word].character_size; c++) {
					  ...
					  vocab[last_word].char_ids[c] = charv_id;
					  for (d = 0; d < dim; d++){
						  neu1char[d] += charv[d + charv_id * dim] / vocab[last_word].character_size;
						  cvec[d + cw * dim] = charv[d + charv_id * dim] / vocab[last_word].character_size;
					  }
				  ...
				  if (ada_com >= 0){
					  weights[cw] = 0;
					  weights[cw] += weit[last_word];

					  for (c = 0; c < vocab[last_word].character_size; c++){
						  charv_id = vocab[last_word].char_ids[c];
						  weights[cw] += weit[vocab_size + charv_id];
					  }
                      //printf("ok2.5\n");fflush(stdout);
                      //printf("vocab[*].cn: %lld\tweit[*]: %lf\n", vocab[last_word].cn, weit[vocab_size + character_size]);fflush(stdout);
					  ///weights[cw] += weit[vocab_size + character_size] * log(vocab[last_word].cn);
                      //getchar();
                      //printf("weights[cw]: %lf\n", weights[cw]); fflush(stdout);

                      if(weights[cw] > MAX_EXP) weights[cw] = 1;
					  else if(weights[cw] < -MAX_EXP) weights[cw] = 0;
                      else weights[cw] = expTable[(int)((weights[cw] + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
                      
                      //printf("weights[cw]: %lf\n", weights[cw]); fflush(stdout);
                      //printf("ok2.75\n"); fflush(stdout);
					  for (c = 0; c < dim; c++) neu1char[c] = 0;
					  for (c = 0; c < dim; c++) {
						  neu1char[c] = weights[cw] * syn0[c + last_word * dim];
					  }
					  ...
					  if (cwe_type && vocab[last_word].character_size) {
						  for (c = 0; c < vocab[last_word].character_size; c++) {
							  charv_id = vocab[last_word].char_ids[c];
							  for (d = 0; d < dim; d++){
								  neu1char[d] += (1 - weights[cw]) * charv[d + charv_id * dim] / vocab[last_word].character_size;
							  }
						  }
					  }
					  //printf("ok4\n"); fflush(stdout);
              ...
              //printf("neu1[*]:%lf\tneu1[*]:%lf\n", neu1[0], neu1[10]);fflush(stdout);
			  cw++;
		  }
		  if (cw) {
			  ...
			  for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
				  ...
				  for (c = 0; c < dim; c++) syn0[c + last_word * dim] += weights[cwcp] * neu1e[c] / cw; // minor bug in original code.
				  ...
				  if (ada_com >= 0){
					  // update weit
					  delta_alpha = 0;
					  for (c = 0; c < dim; c++) delta_alpha += neu1e[c] * (cvec[c + cwcp * dim] - nvec[c + cwcp * dim]);
					  delta_alpha *= weights[cwcp] * (1 - weights[cwcp]) / cw;
                      //printf("neu1e[*]: %lf\tcvec[*]: %lf\tnvec[*]:%lf\n", neu1e[10], cvec[10 + cwcp * dim], nvec[c + cwcp * dim]); fflush(stdout);

					  weit[last_word] += delta_alpha;
					  for (c = 0; c < vocab[last_word].character_size; c++){
						  charv_id = vocab[last_word].char_ids[c];
						  weit[vocab_size + charv_id] += delta_alpha;
					  }
					  //
                      //getchar(); 
					  //weit[vocab_size + character_size] += delta_alpha * log(vocab[last_word].cn);
                      //printf("delt_alpha: %lf\tvocab[last_word].cn: %lld\tweit[*]: %lf\n", delta_alpha, vocab[last_word].cn, weit[vocab_size + character_size]); fflush(stdout);
				  }
				  cwcp++;
			  ...
			  if (ada_com < 0){
				  ...
				  for (a = 0; a < char_list_cnt; a++) {
					  ...
					  for (c = 0; c < dim; c++)
					  {
						  charv[c + charv_id * dim] += (1 - weights[cwcp]) * neu1e[c] * char_rate / cw ;
					  }
				  }
				  //printf("ok8\n"); fflush(stdout);
			  }
			  ...
			  for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
				  ...
				  if (cwe_type && vocab[last_word].character_size) {
					  ...
					  if (ada_com >= 0){
						  weight = 0;
						  weight += weit[last_word];

						  for (c = 0; c < vocab[last_word].character_size; c++){
							  charv_id = vocab[last_word].char_ids[c];
							  weight += weit[vocab_size + charv_id];
						  }
						  weight += weit[vocab_size + character_size] * log(vocab[last_word].cn);
						  if (weight > MAX_EXP) weight = 1;
                          else if (weight < -MAX_EXP) weight = 0;
                          else weight = expTable[(int)((weight + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];

						  for (c = 0; c < dim; c++) neu1char[c] = 0;
						  for (c = 0; c < dim; c++) {
							  neu1char[c] = weight * syn0[c + last_word * dim];
						  }
						  ...
						  if (cwe_type && vocab[last_word].character_size) {
							  for (c = 0; c < vocab[last_word].character_size; c++) {
								  charv_id = vocab[last_word].char_ids[c];
								  for (d = 0; d < dim; d++){
									  neu1char[d] += (1 - weight) * charv[d + charv_id * dim] / vocab[last_word].character_size;
								  }
							  }
						  }
						  for (c = 0; c < dim; c++)
				  ...
				  if (ada_com < 0){
					  ...
					  for (c = 0; c < dim; c++) syn0[c + l1] += weight * neu1e[c];
					  for (c = 0; c < char_list_cnt; c++){
						  charv_id = charv_id_list[c];
						  for (d = 0; d < dim; d++) charv[d + charv_id * dim] += (1 - weight) * neu1e[d] * char_rate;
					  }
				  }
				  ...
				  if (ada_com >= 0){
					  // update weit
					  delta_alpha = 0;
					  for (c = 0; c < dim; c++) delta_alpha += neu1e[c] * (cvec[c] - nvec[c]);
...
void TrainModel() {
  ...
  wchar_t ch[10];
  char buf[10], pos;
  real *vec = calloc(dim, sizeof(real)), weight;
  FILE *fo;
  ...
  fo = fopen(output_word, "wb");
    ...
    for (a = 0; a < vocab_size; a++) {
      ...
      if (ada_com >= 0){
           ...
           weight += weit[a];
           for (b = 0; b < vocab[a].character_size; b++)
               weight += weit[vocab_size + vocab[a].char_ids[b]];
           //weight += weit[vocab_size + character_size] * log(vocab[a].cn);
           if (weight > MAX_EXP) weight = 1;
           else if (weight < -MAX_EXP) weight = 0;
           else weight = expTable[(int)((weight + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))];
           
           for (b = 0; b < dim; b++) vec[b] = weight * syn0[b + a *dim];
           if (cwe_type && vocab[a].character_size) {
              for (b = 0; b < vocab[a].character_size; b++){
                charv_id = vocab[a].char_ids[b];
                for (c = 0; c < dim; c++) 
                    vec[c] += (1 - weight) * charv[c + charv_id * dim] / vocab[a].character_size;
              }
           }
      }
      
      fprintf(fo, "%lf\t", weight);
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\nExamples:\n");
    printf("./cwe -train data.txt -output-word vec.txt -output-char chr.txt -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -iter 3 -ada 0\n\n");
    return 0;
  }
  ...
  if ((i = ArgPos((char *)"-ada", argc, argv)) > 0) ada_com = atoi(argv[i + 1]);
